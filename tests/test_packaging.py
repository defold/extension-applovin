...
class PackagingContractTests(unittest.TestCase):
    ...
    def test_android_extension_r8_keep_rules_are_extension_owned(self) -> None:
        proguard_files = sorted(
            (EXTENSION / "manifests/android").glob("*.pro")
        )
        self.assertEqual([], proguard_files)
        keep_files = sorted(
            (EXTENSION / "manifests/android").glob("*.keep")
        )
        self.assertEqual(
            [EXTENSION / "manifests/android/applovin.keep"], keep_files
        )
        rules = keep_files[0].read_text(encoding="utf-8")
        expected = (
            ...
            "# classes, which are optional when using MAX without LevelPlay.\n"
    ...
    def test_skadnetwork_refresh_skips_retired_mytarget(self) -> None:
        updater = load_ios_updater_module()
        with mock.patch.object(
            updater,
            "fetch_json",
            return_value={
                "company_name": "AppLovin",
                "skadnetwork_ids": [{"skadnetwork_id": "22mmun2rn5.skadnetwork"}],
            },
        ) as fetch:
            identifiers = updater.fetch_adapter_skadnetwork_ids(
                self.catalog, ["22mmun2rn5.skadnetwork"], timeout=1, workers=2
            )
        self.assertEqual([], identifiers["vk"])
        self.assertGreater(fetch.call_count, 0)
        self.assertTrue(
            all("mytarget" not in call.args[0] for call in fetch.call_args_list)
        )
